...
| `EPIC-09` | Soak / robustness / cutover readiness | `complete` | `EPIC-08` | claude | Soak and cutover evidence supports replacement claim | Rust: `shared_daemon_robustness.rs` GREEN (7/7): garbage bytes, oversized msg, garbage payload, partial disconnect, abrupt disconnect, multi-client partial disconnect, rapid 50-cycle churn. Python E2E: `unix/robustness.py` (10/10), `tcp/robustness.py` (13/13), `unix/soak.py` (9/9), `tcp/soak.py` (9/9), `unix/cutover.py` (8/8), `tcp/cutover.py` (8/8). RSS growth bounded (<1.2x after 100 churn cycles). Cutover/rollback dry runs validated. Workspace tests pass (799). |
...
| `EPIC-09-01` | `EPIC-09` | `done` | claude | Rust: `shared_daemon_robustness.rs` (7 tests: garbage bytes, oversized msg, garbage payload, partial disconnect, abrupt disconnect, multi-client partial disconnect, rapid 50-cycle churn). Python E2E: `unix/robustness.py` (10/10), `tcp/robustness.py` (13/13), `unix/soak.py` (9/9), `tcp/soak.py` (9/9), `unix/cutover.py` (8/8), `tcp/cutover.py` (8/8) | Phase 0: SessionRegistry O(1) reverse index, snapshot dirty flag, Python helper extensions. Phase 1: 7 Rust robustness integration tests. Phase 2: Unix+TCP robustness (garbage, partial HDLC, oversized, crash client, half-open, rapid churn, invalid auth). Phase 3: Unix+TCP soak (100-cycle churn, mixed workload, restart during churn, RSS monitoring). Phase 4: Unix+TCP cutover/rollback dry runs (rnsd→rete-shared→rnsd). Operator docs: CUTOVER_CHECKLIST.md, ROLLBACK_CHECKLIST.md. Container runner updated with robustness/soak/cutover suites. All 16 S3 parity rows covered. Workspace tests pass (799). |
...
10. ~~`EPIC-09` Soak / robustness / cutover readiness.~~ COMPLETE — **Program complete.**
