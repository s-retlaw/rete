...
# --- Phase 1: Python sends large resource to Rust ---
...
if not resource_sent.wait(timeout=60):
...
# --- Phase 2: Rust sends large resource to Python ---
...
if resource_received.wait(timeout=60):
...
""")
        ...
        while time.monotonic() < deadline:
            ...
            # Send large resource from Rust when Python is ready
        ...
        # --- Assertion 2: Python->Rust large resource transfer ---
        rust_resource_complete_lines = [l for l in rust if l.startswith("RESOURCE_COMPLETE:")]
        py_to_rust_ok = any("MW_PY_" in l for l in rust_resource_complete_lines)
        if py_to_rust_ok:
            # Verify data integrity — check received data matches
