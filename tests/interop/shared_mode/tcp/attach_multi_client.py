#!/usr/bin/env python3
"""S1-TCP-ATTACH-002: Two stock Python clients attach to rete-shared over TCP.

Topology:
  Rust daemon (rete-shared) in TCP shared mode
  + 2 stock Python RNS clients with share_instance=Yes, shared_instance_port set

Usage:
  cargo build -p rete-daemon --bin rete-shared
  cd tests/interop
  uv run python shared_mode/tcp/attach_multi_client.py [--rust-binary PATH]
"""
...
PORT = 49000 + (os.getpid() % 1000) + 1


def main():
    args = parse_args()
    t = SharedModeTest("S1-TCP-ATTACH-002", rust_binary=args.rust_binary)
