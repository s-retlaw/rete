#!/usr/bin/env python3
"""S2-TCP-OPER-002: Full rnstatus-style queries against rete-shared (TCP).
...
Usage:
  ...
  cd tests/interop
  uv run python shared_mode/tcp/rnstatus_full.py [--rust-binary PATH]
"""
...
def main():
    args = parse_args()
    t = SharedModeTest("S2-TCP-OPER-002", rust_binary=args.rust_binary)
    ...
    try:
        ...
        # 1. Query interface_stats
        print("Querying interface_stats via TCP...")
        ...
        # 2. Query path_table
        print("Querying path_table via TCP...")
        pt_response = rpc_query_tcp("127.0.0.1", CTRL_PORT, authkey, {"get": "path_table"})
        t.check(pt_response is not None, "path_table response received")
        if pt_response is not None:
            t.check(isinstance(pt_response, dict), "path_table is a dict")
            t.check(len(pt_response) > 0, f"path_table has entries (got {len(pt_response)})")

        # 3. Query link_count
        print("Querying link_count via TCP...")
        lc_response = rpc_query_tcp("127.0.0.1", CTRL_PORT, authkey, {"get": "link_count"})
        t.check(lc_response is not None, "link_count response received")
        if lc_response is not None:
            t.check(isinstance(lc_response, int), f"link_count is int (got {type(lc_response).__name__})")
