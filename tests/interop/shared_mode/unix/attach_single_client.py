#!/usr/bin/env python3
"""S1-UNX-ATTACH-001: Single stock Python client attaches to rete-shared over Unix.

Topology:
  Rust daemon (rete-shared) in Unix shared mode
  + 1 stock Python RNS client with share_instance=Yes

Usage:
  cargo build -p rete-daemon --bin rete-shared
  cd tests/interop
  uv run python shared_mode/unix/attach_single_client.py [--rust-binary PATH]
"""
...
def main():
    args = parse_args()
    t = SharedModeTest("S1-UNX-ATTACH-001", rust_binary=args.rust_binary)

    try:
        print("Starting rete-shared daemon (Unix mode)...")
        t.start_daemon()

        client_dir = t.make_client_dir("client1")
