#!/usr/bin/env python3
"""S1-UNX-ATTACH-003: Client reconnects to rete-shared after drop.
...
Usage:
  ...
  cd tests/interop
  uv run python shared_mode/unix/reconnect.py [--rust-binary PATH]
"""
...
def main():
    args = parse_args()
    t = SharedModeTest("S1-UNX-ATTACH-003", rust_binary=args.rust_binary)
    ...
    try:
        ...
        crash_dir = t.make_client_dir("crash_client")
        ...
        # Wait for daemon to detect disconnect (EOF on socket close).
        time.sleep(0.5)
        t.check(
        ...
        reconnect_dir = t.make_client_dir("reconnect_client")
